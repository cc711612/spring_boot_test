...
import com.roy.spring_boot_mall.rowmapper.UserRowMapper;
...
public class UserDaoImpl implements UserDao {
    ...
    public User getUserById(Integer userId) {
        String sql = "SELECT user_id,email,created_date,last_modified_date FROM user Where `user_id` = :userId";
        Map<String, Object> map = new HashMap<>();
    ...
    {
        String sql = "SELECT user_id,email,created_date,last_modified_date FROM user Where `email` = :email";
        Map<String, Object> map = new HashMap<>();
        map.put("email", email);
        List<User> userList = namedParameterJdbcTemplate.query(sql, map, new UserRowMapper());
        if (userList.size() > 0) {
